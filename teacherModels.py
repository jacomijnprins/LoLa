...
def train_models(meta_files):
    ...
    for idx, file_path in enumerate(meta_files):
        print(f"Training on meta-dataset {idx + 1} from file: {file_path}")
        ...
        training_args = TrainingArguments(
            output_dir=f'models/meta_{idx + 1}_results',  # Save model and logs here
            eval_strategy='no',
            learning_rate=2e-5,
            ...
            logging_dir=f'models/meta_{idx + 1}_logs',  # TensorBoard logs
            logging_steps=10
        )
        ...
        trainer.train()
        trainer.save_model(f"models/meta_{idx + 1}_model")

        print(f"Model for meta-dataset {idx + 1} saved to models/meta_{idx + 1}_model")
